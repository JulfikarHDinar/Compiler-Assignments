...
void lexemeSeparator2(FILE *source, FILE *target, char sourceFileName[30], char targetFileName[30]){
    char c;
    int lexemeStart = 0;
    ...
    if(source == NULL || target == NULL || temp == NULL){
        ...
        while( (c = fgetc(source)) != EOF ){
            ...
            if(lexemeStart == 1){
                if(c == ';' || c == ',' || c == 39 || c == '+' || c == '-' || c == '*' || c == '/' || c == '%' || c == '(' || c == ')' || c == '{' || c == '}'){    //apostrophe ascii = 39
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////////
void lexemeTokenization2(FILE *source, FILE *target, char sourceFileName[30], char targetFileName[30]){
    char c, lex[50];
    if(source == NULL || target == NULL){
        ...
        while( (c = fgetc(source)) != EOF ){
            if(c == ' '){       //checking each lexeme separately and putting them in target file
                ...
                else if(isParenthesis2(lex)){
                    fputs("[par ", target);
                    fputs(lex, target);
                    fputs("] ", target);
                }
                else if(isBracket2(lex)){
                    fputs("[brc ", target);
                    fputs(lex, target);
                    fputs("] ", target);
                }
                else if(isOperator2(lex)){
                    fputs("[op ", target);
                    fputs(lex, target);
                    fputs("] ", target);
                }
                else if(isNumerical2(lex)){
                    fputs("[num ", target);
                    fputs(lex, target);
                    fputs("] ", target);
                }
                else if(isKeyword2(lex)){
                    fputs("[kw ", target);
                    fputs(lex, target);
                    fputs("] ", target);
                }
                else if(isIdentifier2(lex)){
                    fputs("[id ", target);
                    fputs(lex, target);
                    fputs("] ", target);
                }
